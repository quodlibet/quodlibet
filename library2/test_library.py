...
class TLibrary(unittest.TestCase):
    ...
    def test_add(self):
        self.library.add(Frange(12))
        self.failUnlessEqual(self.added, Frange(12))
        del(self.added[:])
        self.library.add(Frange(12, 24))
        self.failUnlessEqual(self.added, Frange(12, 24))

    def test_remove(self):
        self.library.add(Frange(10))
        self.library.remove(Frange(3, 6))
        self.failUnlessEqual(self.removed, Frange(3, 6))

        # Neither the objects nor their keys should be present.
        self.failIf(Fake(3) in self.library)
        self.failUnless(Fake(6) in self.library)
        self.failIf(3 in self.library)
        self.failUnless(6 in self.library)

    def test_remove_when_not_present(self):
        self.failUnlessRaises(KeyError, self.library.remove, [Fake(12)])

    def test_changed(self):
        self.library.add(Frange(10))
        self.library.changed(Frange(5))
        self.failUnlessEqual(self.changed, Frange(5))

    def test_changed_not_present(self):
        self.library.add(Frange(10))
        self.library.changed(Frange(2, 20, 3))
    ...
    def test___iter__(self):
        self.library.add(Frange(10))
        self.failUnlessEqual(sorted(list(self.library)), Frange(10))
...
class TLibrarian(unittest.TestCase):
    def setUp(self):
        ...
        self.lib1 = Library("One")
        self.lib2 = Library("Two")
    ...
    def test_register_exists(self):
        self.failUnlessRaises(ValueError, Library, "Two")
    ...
    def test_added(self):
        self.lib1.add(Frange(12))
        self.lib2.add(Frange(12, 24))
        self.failUnlessEqual(sorted(self.added), Frange(24))

    def test_removed(self):
        self.lib1.add(Frange(12))
        self.lib2.add(Frange(12, 24))
        self.lib1.remove([Fake(9)])
        self.lib2.remove([Fake(16)])
        self.failUnlessEqual(self.removed, [Fake(9), Fake(16)])

    def test_changed(self):
        self.lib1.add(Frange(12))
        self.lib2.add(Frange(12, 24))
        self.librarian.changed(Frange(6, 18))
        self.failUnlessEqual(sorted(self.changed), Frange(6, 18))
        self.failUnlessEqual(self.changed_1, Frange(6, 12))
        self.failUnlessEqual(self.changed_2, Frange(12, 18))
