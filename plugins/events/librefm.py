...
class QLLibrefm(EventPlugin):
    ...
    try: DUMP = os.path.join(const.USERDIR, "libre_cache")
        ...
        DUMP = os.path.join(const.DIR, "libre_cache")
    ...
    def read_config(self):
        ...
        password = ""
        try:
            username = config.get("plugins", "libre_username")
            password = config.get("plugins", "libre_password")
        except:
            if (self.need_config == False and
                getattr(self, 'PMEnFlag', False)):
                self.quick_dialog("Please visit the Preferences window to set QLLibrefm up. Until then, songs will not be submitted.", gtk.MESSAGE_INFO)
                self.need_config = True
                return

        try: self.offline = (config.get("plugins", "libre_offline") == "true")
        except: pass
        try: self.exclude = config.get("plugins", "libre_exclude")
        except: pass
    ...
    def quick_dialog_helper(self, type, str):
        dialog = Message(gtk.MESSAGE_INFO, widgets.main, "QLLibrefm", str)
    ...
    def send_handshake(self):
        # construct url
        url = "http://post.libre.fm/?hs=true&p=%s&c=%s&v=%s&u=%s" % ( self.PROTOCOL_VERSION, self.CLIENT, self.PLUGIN_VERSION, self.username )
        
        log("Sending handshake to Libre.fm.")
        ...
        if status == "UPTODATE" or status.startswith("UPDATE"):
            if status.startswith("UPDATE"):
                self.quick_dialog("A new plugin is available at %s! Please download it, or your Libre.fm stats may not be updated, and this message will be displayed every session." % status.split()[1], gtk.MESSAGE_INFO)
...
#            resp = urllib2.urlopen("http://" + host + "/" + file, data_str)
            ...
            log("Libre.fm server not responding, will try later.")
            self.locked = False
        ...
        if status == "BADAUTH":
            ...
            self.send_handshake()
            if self.challenge_sent == False:
                self.quick_dialog("Your Libre.fm login data is incorrect, so you must re-enter it before any songs will be submitted.\n\nThis message will not be shown again.", gtk.MESSAGE_ERROR)
                self.broken = True
        elif status == "OK":
    ...
    def PluginPreferences(self, parent):
        def toggled(widget):
            if widget.get_active():
                config.set("plugins", "libre_offline", "true")
                self.offline = True
            else:
                config.set("plugins", "libre_offline", "false")
                self.offline = False

        def changed(entry, key):
            # having a function for each entry is unnecessary..
            config.set("plugins", "libre_" + key, entry.get_text())
        ...
        def destroyed(*args):
            ...
            newp = None
            try:
                newu = config.get("plugins", "libre_username")
                newp = config.get("plugins", "libre_password")
            except:
                return
        ...
        try: userent.set_text(config.get("plugins", "libre_username"))
        except: pass
        try: pwent.set_text(config.get("plugins", "libre_password"))
        except: pass
        try:
            if config.get("plugins", "libre_offline") == "true":
                off.set_active(True)
        except: pass
        try: ve.set_text(config.get("plugins", "libre_exclude"))
        except: pass
