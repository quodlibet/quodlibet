...
class Preferences(gtk.VBox):
    ...
    def __init__(self):
        ...
        for idx, key in enumerate(["level", "band", "width"]):
            ...
            step = steps[idx]
            page = pages[idx]
            scale = gtk.HScale(gtk.Adjustment(0, 0, max_value, step, page))
            scales[key] = scale
            if step < 0.1:
                scale.set_digits(2)
            scale.add_mark(_SETTINGS[key][2], gtk.POS_BOTTOM, None)
...
class Karaoke(GStreamerPlugin):
    ...
    def setup_element(cls):
        try:
            return gst.element_factory_make('audiokaraoke', cls.PLUGIN_ID)
